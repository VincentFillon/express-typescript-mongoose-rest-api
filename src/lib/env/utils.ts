...
export function getPath(path: string): string {
  return process.env.NODE_ENV === 'production' ? join(process.cwd(), path.replace('src/', 'dist/').slice(0, -3) + '.js') : join(process.cwd(), path);
}
...
export function getOsEnvArray(key: string, delimiter: string = ','): string[] {
  const envArray = process.env[key];
  return envArray ? envArray.split(delimiter) : [];
}
